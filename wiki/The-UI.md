...
On an Enterprise account with an organization spend limit, the dashboard's two bars relabel from **5-HOUR/7-DAY** to **SPEND/PERIOD** — same layout, different meaning, with PERIOD's trailing number becoming a projected end-of-period spend instead of a countdown. See [How it works](How-It-Works#rate-limit-headers-and-your-plan).
...
- **Four screens** — the Mango dashboard, a **7-day usage chart** (one sample per 30 min, gaps where the device was off, day markers at local midnight, tracking whichever bar pair — 5h/7d or spend/period — the account is currently emitting, and clearing itself if that changes), **Anthropic news** headlines, and a **desk clock** (local time + micro usage bars)
